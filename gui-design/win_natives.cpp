...
static void Win_editorApplySpans(Dart_NativeArguments args) {
  ...
  intptr_t n = 0; Dart_ListLength(runs, &n);
  (void)wdg;
  for (intptr_t i = 0; i + 2 < n; i += 3) {
    ...
    // RichEdit: select [start,start+len), set CHARFORMAT crTextColor = ColorForKind(kind).
    // kind: 1 keyword 2 string 3 comment 4 number 5 type, else default
    // (cocoa_callbacks.mm:253-262).
  }
}

// ── Canvas / game (bodies are S5/S6; enumerated for the resolver only) ───────
// _canvasBlit(int ticket, TypedData px, int w, int h) — Direct2D/WIC blit behind
// the unchanged demo draw protocol.
static void Win_canvasBlit(Dart_NativeArguments args) { /* S5 */ }
// Game pane — SAME 7-native shape as cocoa_natives.mm:502-508 (D3D11 in S6).
static void Win_gpOpen(Dart_NativeArguments args)       { /* S6 */ }
static void Win_gpClose(Dart_NativeArguments args)      { /* S6 */ }
static void Win_gpApply(Dart_NativeArguments args)      { /* S6 */ }
static void Win_gpSnap(Dart_NativeArguments args)       { /* S6 */ }
static void Win_gpStat(Dart_NativeArguments args)       { /* S6 */ }
static void Win_gpFullscreen(Dart_NativeArguments args) { /* S6 */ }
static void Win_gpBackbuffer(Dart_NativeArguments args) { /* S6 */ }
